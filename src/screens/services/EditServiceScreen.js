...
export default function EditServiceScreen({ navigation, route }) {
  ...
  const [form, setForm] = useState({
    ...
    price: '', price_type: 'fixed', state: '', city: '',
    address: '', phone: '', whatsapp: '', is_available: true,
  });
  ...
  const uploadNewImages = async () => {
    ...
    for (const img of images) {
      ...
      const { error } = await supabase.storage.from('service-images').upload(fileName, arrayBuffer, { contentType: mimeType });
      if (!error) {
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.availRow}>
            <View>
              <Text style={styles.availLabel}>Service available</Text>
            ...
            {SERVICE_CATEGORIES.map((cat) => (
              ...
              >
                <Ionicons
                  ...
                  size={16}
                  color={form.category_id === cat.id ? COLORS.textOnGold : COLORS.textSecondary}
                />
        ...
        <View style={styles.section}>
          ...
          <Field label="Address (optional)">
            <TextInput style={styles.input} placeholderTextColor={COLORS.textMuted}
              placeholder="e.g. 12 Allen Avenue" value={form.address} onChangeText={(v) => update('address', v)} />
          </Field>
        </View>
...
const styles = StyleSheet.create({
  ...
  // ── Pricing ──
  priceTypeRow: { flexDirection: 'row', gap: 8, marginBottom: 14 },
  priceTypeBtn: {
  ...
  priceTypeBtnText: { fontSize: 12, fontWeight: '600', color: COLORS.textSecondary },
